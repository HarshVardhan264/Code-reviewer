...
// Native WebGPU compatibility layer.
// Keeps AeroShards' shaders, settings, interaction model, and visual pipeline intact.
...
class NativeUniforms {
  constructor(device, byteLength) {
    this.device = device;
    this.buffer = device.createBuffer({ size: Math.ceil(byteLength / 16) * 16, usage: GPUBufferUsage.UNIFORM | GPUBufferUsage.COPY_DST });
  }
  set(values) {
    const parts = Object.values(values);
    const data = new Float32Array(parts.length * 4);
    let offset = 0;
    for (const value of parts) { data.set(value, offset); offset += 4; }
    this.device.queue.writeBuffer(this.buffer, 0, data);
  }
}

class NativeTarget {
  constructor(device, descriptor, context = null) {
    this.device = device; this.context = context; this.format = descriptor.format || 'rgba8unorm';
    this.size = descriptor.size || [1, 1]; this.texture = null;
    this.label = descriptor.label || 'aero-target';
    if (!context) this.resize(this.size);
  }
  resize(size) {
    this.size = [Math.max(1, Math.round(size[0])), Math.max(1, Math.round(size[1]))];
    if (this.context) return;
    this.texture?.destroy();
    this.texture = this.device.createTexture({
      size: [this.size[0], this.size[1], 1], format: this.format,
      usage: GPUTextureUsage.RENDER_ATTACHMENT | GPUTextureUsage.TEXTURE_BINDING,
      label: this.label
    });
  }
  view() { return this.context ? this.context.getCurrentTexture().createView() : this.texture.createView(); }
}

class NativeSampler {
  constructor(device, descriptor) { this.sampler = device.createSampler({ magFilter: descriptor.magFilter, minFilter: descriptor.minFilter, addressModeU: descriptor.addressModeU, addressModeV: descriptor.addressModeV }); }
}

class NativeEffect {
  constructor(device, shader, descriptor = {}) {
    this.device = device; this.shader = shader; this.resources = descriptor.set || {}; this.label = descriptor.label || 'aero-effect';
    this.pipeline = null; this.pipelines = new Map(); this.format = null; this.bindGroup = null; this.signature = '';
  }
  set(values) { Object.assign(this.resources, values); this.bindGroup = null; }
  async compile(targetOrDescriptor) {
    const format = targetOrDescriptor?.format || targetOrDescriptor?.colors?.[0] || 'rgba8unorm';
    this.format = format;
    if (this.pipelines.has(format)) { this.pipeline = this.pipelines.get(format); return; }
    const module = this.device.createShaderModule({ code: FULLSCREEN_VERTEX + this.shader });
    this.pipeline = await this.device.createRenderPipelineAsync({
      label: this.label, layout: 'auto',
      vertex: { module, entryPoint: 'vs_main' },
      fragment: { module, entryPoint: 'fs_main', targets: [{ format, blend: { color: { srcFactor:'one', dstFactor:'zero', operation:'add' }, alpha: { srcFactor:'one', dstFactor:'zero', operation:'add' } } }] },
      primitive: { topology: 'triangle-list' }
    });
    this.pipelines.set(format, this.pipeline);
  }
  getPipeline(format = this.format) { return this.pipelines.get(format) || this.pipeline; }
  getBindGroup() {
    const entries = [];
    let binding = 0;
    for (const value of Object.values(this.resources)) {
      if (value instanceof NativeTarget) entries.push({ binding: binding++, resource: value.view() });
      else if (value instanceof NativeSampler) entries.push({ binding: binding++, resource: value.sampler });
      else if (value instanceof NativeUniforms) entries.push({ binding: binding++, resource: { buffer: value.buffer } });
    }
    return this.device.createBindGroup({ layout: this.pipeline.getBindGroupLayout(0), entries });
  }
}

class NativeDraw extends NativeEffect {
  async compile(targetOrDescriptor) {
    const format = targetOrDescriptor?.format || targetOrDescriptor?.colors?.[0] || 'rgba8unorm';
    this.format = format;
    if (this.pipelines.has(format)) { this.pipeline = this.pipelines.get(format); return; }
...
class NativePass {
  constructor(encoder, descriptor) {
    this.descriptorFormat = descriptor.target.format;
    this.pass = encoder.beginRenderPass({
      colorAttachments: [{ view: descriptor.target.view(), clearValue: descriptor.clear || [0,0,0,1], loadOp: 'clear', storeOp: 'store' }]
    });
  }
  draw(drawable, options = {}) {
    this.pass.setPipeline(drawable.getPipeline(this.descriptorFormat));
    this.pass.setBindGroup(0, drawable.getBindGroup());
    this.pass.draw(drawable instanceof NativeDraw ? 6 : 3, options.instances || 1);
  }
  end() { this.pass.end(); }
}

class NativeFrame {
  constructor(device) { this.encoder = device.createCommandEncoder(); }
  pass(descriptor, callback) { const pass = new NativePass(this.encoder, descriptor); callback(pass); pass.end(); }
}
...
const nativeInit = async () => {
  ...
  device.lost.then(info => {
    if (info?.reason !== 'destroyed') {
      console.error('[AeroShards] WebGPU device lost:', info);
    }
  });
  return {
    device,
...
const nativeSurface = (gpu, canvas, options) => {
  const context = canvas.getContext('webgpu');
  if (!context) throw new Error('Unable to create a WebGPU canvas context.');
  context.configure({ device: gpu.device, format: options.format, alphaMode: 'premultiplied' });
  const initialDpr = options.dpr || window.devicePixelRatio || 1;
  const initialSize = [Math.max(1, Math.round(canvas.clientWidth * initialDpr)), Math.max(1, Math.round(canvas.clientHeight * initialDpr))];
  canvas.width = initialSize[0]; canvas.height = initialSize[1];
  const output = new NativeTarget(gpu.device, { size: initialSize, format: options.format }, context);
  output.resize = size => {
    output.size = [Math.max(1, Math.round(size[0])), Math.max(1, Math.round(size[1]))];
    canvas.width = output.size[0]; canvas.height = output.size[1];
  };
  output.onResize = callback => { output._resizeCallback = callback; return () => {}; };
  return output;
};

const nativeDraw = (gpu, descriptor) => new NativeDraw(gpu.device, descriptor.shader, descriptor);
const nativeEffect = (gpu, shader, descriptor) => new NativeEffect(gpu.device, shader, descriptor);
const nativeTarget = (gpu, descriptor) => new NativeTarget(gpu.device, descriptor);
const nativeSampler = (gpu, descriptor) => new NativeSampler(gpu.device, descriptor);
const nativeUniforms = (gpu, values) => new NativeUniforms(gpu.device, Object.keys(values).length * 16);
const nativeFrame = (gpu, callback) => {
  const frame = new NativeFrame(gpu.device); callback(frame); gpu.device.queue.submit([frame.encoder.finish()]);
};
...
const createRenderGraph = (gpu, outputSize, bloomSize = resolveBloomSize(outputSize)) => {
  const viewParams = nativeUniforms(gpu, {
    ...
    highlightColor: mixColor([201 / 255, 138 / 255, 75 / 255, 1], [1, 1, 1, 1], MATERIAL_PRESETS.pearl.highlightMix),
    accentColor: [201 / 255, 138 / 255, 75 / 255, 1]
  });
  const postParams = nativeUniforms(gpu, {
    ...
    background: [0.031, 0.024, 0.016, 1],
    temporal: [0, 0, 0, 0],
    tint: [137 / 255, 106 / 255, 189 / 255, 1]
  });
  const shardDraw = nativeDraw(gpu, {
    shader: SHARD_SHADER,
    vertices: 6,
    blend: 'premultiplied',
    cull: 'none',
  ...
  const sceneTarget = nativeTarget(gpu, {
    size: outputSize,
    format: 'rgba8unorm',
    label: 'aero-shards-scene'
  });
  const bloomTarget = nativeTarget(gpu, {
    size: bloomSize,
    format: 'rgba16float',
    label: 'aero-shards-bloom'
  });
  const bloomScratchTarget = nativeTarget(gpu, {
    size: bloomSize,
    format: 'rgba16float',
    label: 'aero-shards-bloom-scratch'
  });
  const linearSampler = nativeSampler(gpu, {
    minFilter: 'linear',
    magFilter: 'linear',
    addressModeU: 'clamp-to-edge',
    addressModeV: 'clamp-to-edge'
  });
  const bloomEffect = nativeEffect(gpu, BLOOM_SHADER, {
  ...
  const blurParamsX = nativeUniforms(gpu, { direction: [1 / bloomSize[0], 0, 0, 0] });
  const blurParamsY = nativeUniforms(gpu, { direction: [0, 1 / bloomSize[1], 0, 0] });
  const bloomBlurX = nativeEffect(gpu, BLOOM_BLUR_SHADER, {
    label: 'aero-shards-bloom-horizontal',
    set: { bloomTexture: bloomTarget, linearSampler, blur: blurParamsX }
  });
  const bloomBlurY = nativeEffect(gpu, BLOOM_BLUR_SHADER, {
    label: 'aero-shards-bloom-vertical',
    set: { bloomTexture: bloomScratchTarget, linearSampler, blur: blurParamsY }
  });
  const finishEffect = nativeEffect(gpu, FINISH_SHADER, {
  ...
  const styleTarget = nativeTarget(gpu, { size: [1, 1], format: 'rgba8unorm', label: 'aero-shards-style' });
  const asciiTarget = nativeTarget(gpu, { size: [1, 1], format: 'rgba8unorm', label: 'aero-shards-ascii-cells' });
  const styleParams = nativeUniforms(gpu, {
    viewport: [outputSize[0], outputSize[1], 6, 10],
    background: [0.031, 0.024, 0.016, 1],
    mode: [0, 0, 0, 0]
  });
  const asciiEffect = nativeEffect(gpu, ASCII_CELL_SHADER, {
    label: 'aero-shards-ascii-match',
    set: { sourceTexture: sceneTarget, sourceSampler: linearSampler, style: styleParams }
  });
  const styleEffect = nativeEffect(gpu, STYLE_SHADER, {
...
export default function AeroShards({
  ...
  const resolvedBackground = parseColor(backgroundColor, '#080604');
  const resolvedShardColor = parseColor(shardColor, '#C98A4B');
  const resolvedAccentColor = parseColor(accentColor, '#C98A4B');
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const output = nativeSurface(gpu, canvas, {
